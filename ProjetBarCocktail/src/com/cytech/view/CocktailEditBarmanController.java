...
public class CocktailEditBarmanController {
    ...
    public void setMainApp(MainApp mainApp) {
        ...
        cocktail =  mainApp.getConnectedBarman().creerCocktailBarman(nomCocktail.getText(), mapBoisson, mapSupplement);
    }
    ...
    private void handleAjouterBA() {
		...
		if (boissonSelec == null || contenanceSelecString == null)
		{
			...
			cocktail.ajouterBoissonAlcoolisee(boissonSelec, contenanceSelec);
    ...
    private void handleAjouterBNA() {
		...
		if (boissonSelec == null || contenanceSelecString == null)
		{
			//TODO: mettre la boisson d'indice zero ?
			Alert alert = new Alert(AlertType.WARNING);
			alert.initOwner(mainApp.getPrimaryStage());
			alert.setTitle("Pas de sélection");
			alert.setHeaderText("Erreur de sélection");
			...
			cocktail.ajouterBoissonNonAlcoolisee(boissonSelec, contenanceSelec);
    ...
    private void handleAjouterS() {
		...
		if (supplementSelec == null || quantiteSelecString == null)
		{
			...
			cocktail.ajouterSupplement(supplementSelec, quantiteSelec);
    ...
    private void handleTerminer() {
    	cocktail.calculerContenance();
    	cocktail.calculerSucre();
    	cocktail.calculerDegreAlcool();
    	cocktail.calculerPrix();
    	cocktail.setNom(nomCocktail.getText());
    	GestionCocktail.ecritureCocktail(cocktail);
    	mainApp.initCollections();
    	dialogStage.close();
